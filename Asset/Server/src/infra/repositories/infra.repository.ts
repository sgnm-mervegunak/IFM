...
export class InfraRepository implements InfraInterface {
  ...
  async importClassificationFromExcel(file: Express.Multer.File, language: string) {
    try {
    let checkClassification1 = await this.neo4jService.findByLabelAndFilters([`DurationUnits_${language}`],{realm:"Signum"})
    let checkClassification2 = await this.neo4jService.findByLabelAndFilters([`AssetType_${language}`],{realm:"Signum"})
    let checkClassification3 = await this.neo4jService.findByLabelAndFilters([`OmniClass23_${language}`],{realm:"Signum"})
          ...
          for(let i=0;i<classificationsWithoutCode.length;i++){
           ...
           deneme4.push(deneme5);
            ...
            for(let i=0;i < deneme4.length;i++){
              let cypher= `MATCH (n:Classification {realm:"${realmName}"}) MERGE (b:${deneme4[i][0].name}s_${language} {name:"${deneme4[i][0].name}",isDeleted:${deneme4[i][0].isDeleted},key:"${generateUuid()}",realm:"${realmName}",canDelete:false,isActive:true,canCopied:true,isRoot:true,canDisplay:true,language:"${language}"})  MERGE (n)-[:PARENT_OF]->(b)`;
              await this.neo4jService.write(cypher);
    
              for (let index = 1; index < deneme4[i].length; index++) {
    
            let cypher3= `MATCH (n:${deneme4[i][0].name}s_${language} {isDeleted:false}) MERGE (b {code:"${deneme4[i][index].code}",name:"${deneme4[i][index].name}",isDeleted:${deneme4[i][index].isDeleted},key:"${generateUuid()}",canDelete:${deneme4[i][index].canDelete},canDisplay:${deneme4[i][index].canDisplay},language:"${language}"})  MERGE (n)-[:PARENT_OF]->(b)`;
